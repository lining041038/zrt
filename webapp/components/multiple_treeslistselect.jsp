...
function dialogTopIframeShowed(){
	...
	$.getJSON(ctx + "/common/components/MultipleTreesListSelectController/getTreesListSelectOptions.vot?component=" + comp, function(data){
		...
		$("#leftdatagrid").datagrid({
			dataProvider:ctx + "/common/components/MultipleTreesListSelectController/getListSelectItemsToSelect.vot?component=" + comp,
			columns: ncolumns,
			pagebuttonnum: 0,
		...
		if (selectedItemSids || parentsid) {
			$.post(ctx + "/common/components/MultipleTreesListSelectController/getHistSelectedItems.vot?component=" + comp, "selectedItemSids=" + selectedItemSids + "&parentsid=" + parentsid, function (data){
...
%>
	function createLeftGridTree<%=i%>(prevtreesid) {
		...
		var oneTree = $("#treeul<%=i%>").tree("destroy").tree({
			asyncUrl: ctx + "/common/components/MultipleTreesListSelectController/getTreesListSelectItemsToSelect.vot?component=" + comp + "&treekey=<%=entry.getKey()%>" + 
			...
			click: function() {
				...
				if (treeidx == (<%=entries.size()%> - 1)) {
					$("#leftdatagrid").datagrid("option", "dataProvider", ctx + "/common/components/MultipleTreesListSelectController/getListSelectItemsToSelect.vot?component=" + comp + "&treenode_sid=" + treeobj.tree("getSelectedNode").sid);
				} else {
