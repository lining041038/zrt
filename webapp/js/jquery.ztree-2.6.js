...
(function($) {
	...
	function eventProxy(event) {
		...
		if (tools.eqs(event.type, "mouseover")) {
			...
			if (tmp) {
				tId = tmp.parentNode.id;
				if (!setting.expandTreeNodeByClick) {
					childEventType = "switchNode";
				}
			}
		}
	...
	var handler = {
		//点击展开、折叠节点
		onSwitchNode: function (event, onlyopen) {
			var setting = settings[event.data.treeObjId];
			var treeNode = event.data.treeNode;

			if (treeNode.open && onlyopen)
				return;
	...
	function addTreeNodes(setting, parentNode, newNodes, isSilent) {
		...
		if (parentNode) {
			...
			for(var i = 0; i < newNodes.length; i++) {
				if (newNodes[i].open === false)
					continue;
				asyncGetNode(setting, newNodes[i]);
			}
		}
	}
