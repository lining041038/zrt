...
<script>
$(function(){
	$("#mysingleform").closest("form").unbind("resetFormComplete").bind("resetFormComplete", function(){
	//	alert($(".mysinglegridform [name=name]").val($(".mysinglegridform [name=name]").val() + "（js自定义追加）"));
	});
	
	$("#mysingleform").closest("form").rebind("openViewForm", function() {
		resetRoleCheckboxs("view");
	});
	
	$("#mysingleform").closest("form").rebind("openedCreateForm", function() {
		resetRoleCheckboxs("create");
	});
	
	$("#mysingleform").closest("form").rebind("openedUpdateForm", function() {
...
<div id="mysingleform" style="width: 433px; height: 255px;">
	...
	<div class="condi_container">
		...
		<span class="cd_val">
			<div id="rolecheckbox" style="width:189px; positon:relative; top:-20px;">
			</div>
		</span>
	</div>
