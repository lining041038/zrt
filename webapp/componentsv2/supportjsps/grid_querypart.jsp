...
%>
<div class="querypartquerypage" style="display: none; <%=TextUtils.stringSet(request.getParameter("querypartpos"))? request.getParameter("querypartpos"):""%> ">
	...
	<div style="text-align: right; padding-right: 5px; padding-bottom: 5px;">
		<button type="button" onclick="read()" class="btn btn-sm btn-default">${i18n['antelope.query']}</button>
	</div>
</div>

<%if (TextUtils.stringSet(opts.getQueryformKey()) || opts.getQueryfields().length > 0 ){// 确认是否显示查询按钮 %>
<div class="sonbtn_container" style="float: right; position: relative; top: 18px;"> <span class="sonbtn_m_line"></span>
	<button type="button"  onmouseover="onGridpartMouseOver(this)" class="btn btn-sm btn-default sonbtnquerybtn">${i18n['antelope.query']}</button>
</div>
<%} %> 
