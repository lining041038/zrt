...
function icreateViewCall() {
	...
	var ifram = $.dialogTopIframe({
		url: ctx + "/componentsv2/supportjsps/workflow_view.jsp?sid=" + this.sid + "&task_def_keys_=" + this.task_def_keys_ + "&proc_inst_id_=" + this.proc_inst_id_ + "&component=<%=component%>",
		...
		$(".icreated").datagrid({
			...
			buttons: { 
				query: {
			 	...
			 	i_submit: {
			 		visibleFunction: function(obj) {
			 			return "write" == obj.nextopt;
			 		},
			 		toolTip:"打印",
			 		click: function() {
			 			window.open(ctx+"/componentsv2/printtable.jsp?dataurl="+opts.urlprefix+"/dataTable.vot?sid="+this.sid);
			 		}
			 	},i_recall: { // 上传审批附件
			 		visibleFunction: function(obj) {
			 			return "write" == obj.nextopt;
			 		...
			 		click: function() {
			 			...
			 			$("#form2").resetForm(this).submitDialog({
			 				title:"上传审批附件",
			 				url:ctx + "<%=urlprefix%>/endProcess.vot", width:formwidth, height:formheight,
			 				state:"write",
			 				callback: read,
			 				autoTempSaveEnabled: <%=opts.autoTempSaveEnabled%>,
			 				showTempSave:false
			...
			buttons: { 
				release: {
					...
					click:function() {
						top.$("body").css("overflow-y",'hidden');
						var ifram = $.dialogTopIframe({
							url: ctx + "/componentsv2/supportjsps/workflow_auditform.jsp?sid=" + this.sid + "&task_def_key_=" + this.task_def_key_ + 
...
function myDoneViewCall() {
	top.$("body").css("overflow-y",'hidden');
	var ifram = $.dialogTopIframe({
		url: ctx + "/componentsv2/supportjsps/workflow_view.jsp?sid=" + this.sid + "&task_def_keys_=" + this.task_def_keys_ + "&taskid=" + this.id_ + "&task_def_key_=" + this.task_def_key_ +
		...
		$(".alllist").datagrid({
			...
			buttons: { 
				query: {
					click:showAllViewCall
			 	},i_stopchat: { // 导入的项目补充信息
			 		visibleFunction: function(obj) {
			 			return "proopt" == obj.nextopt;
			 		},
			 		toolTip:"完善项目信息",
			 		click: function() {
			 			resetFormWidthHeight();
			 			...
			 			$("#form2").resetForm(this).submitDialog({
			 				title:"完善项目信息",
			 				url:ctx + "<%=urlprefix%>/endProcessImport.vot", width:formwidth, height:formheight,
			 				state:"proopt",
			 				callback: read,
			 				autoTempSaveEnabled: <%=opts.autoTempSaveEnabled%>,
			 				showTempSave:false
			 		...
			 		click: function() {
			 			...
			 			$("#form2").resetForm(this).submitDialog({
			 				title:"修改项目信息",
			 				url:ctx + "<%=urlprefix%>/updateProjectInfo.vot", width:formwidth, height:formheight,
			 				state:"proopt",
			 				callback: read,
			 				autoTempSaveEnabled: <%=opts.autoTempSaveEnabled%>,
			 				showTempSave:false
			...
			buttons: { 
				release: {
					...
					click:function() {
						top.$("body").css("overflow-y",'hidden');
						var ifram = $.dialogTopIframe({
							url: ctx + "/componentsv2/supportjsps/workflow_view.jsp?sid=" + this.sid + "&taskid=" + this.id_ + "&proc_inst_id_=" + this.proc_inst_id_ + "&component=<%=component%>",
...
function read() {
	...
	var selecteidex = $(".workflowtabdiv").tabs("option", "active");
...
function batchComplete() {
	...
	top.$("body").css("overflow-y",'hidden');
	var ifram = $.dialogTopIframe({
		url: ctx + "/componentsv2/supportjsps/batchworkflow_auditform.jsp?&component=<%=component%>",
...
<div class="func_span" style="margin-top: 20px;">
	...
	<button id="batchexportbtn" class="btn btn-sm btn-default" style="height: 28px;line-height: 1.3;" type="button" onClick="batchExportList()" value="">
		<span class="glyphicon glyphicon-export" style="font-family:  Glyphicons Halflings; font-size:12px; padding-right:3px;"></span>
		<%=i18n.get("workflow_datagrids." + component + ".batchexport") %>
	</button>
	<%} %>
	<%if (opts.batchCompleteTasksEnabled) {%>
	<button id="batchcompletebtn" class="btn btn-sm btn-default" style="height: 28px;line-height: 1.3;" type="button" onClick="batchComplete()" value="<%=i18n.get("workflow_datagrids." + component + ".batchcomplete") %>">
		<span class="glyphicon glyphicon-sort-by-attributes" style="font-family:  Glyphicons Halflings; font-size:12px; padding-right:3px;"></span>
		<%=i18n.get("workflow_datagrids." + component + ".batchcomplete") %>
	</button>
	<%} %>
	<%if (opts.showCreateBtn) {%>
	<button class="btn btn-sm btn-default" type="button" style="height: 28px;line-height: 1.3;" onClick="create()" value="<%=i18n.get("workflow_datagrids." + component + ".add") %>">
		<span class="glyphicon glyphicon-plus" style="font-family:  Glyphicons Halflings; font-size:12px; padding-right:3px;"></span>
		<%=i18n.get("workflow_datagrids." + component + ".add") %>
	</button>
	<%} %>
</div>
