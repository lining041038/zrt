...
$(function(){
	...
	// excel处理管理者, 处理包括模板导出以及模板格式验证等工作，为接口
	if (options.excelmanager) {
		$("#exporttmpl").show();
	}
...
function readTmplList() {
	$.getJSON(ctxPath + "/report/gettmpls.vot?groupsid="+(options.groupsid||''), function(data){
		$(".datagrid_container").datagrid("destroy").datagrid({
			...
			buttons:{
				'query': function() {
					$.getJSON(ctxPath + "/report/gettmplinfo.vot?sid=" + this.sid, function(data) {
						$("[name=tmplneweditform]").resetForm(data);
						setHtmltable(data.htmltable);
						openTmplEditDialog(data.isrelease);
					});
				}, 'release': {
					visibleFunction: function () {
						...
						if (confirm("模板发布之后将不允许重新修改，确认要发布吗？")) {
							$.get(ctxPath + "/report/release.vot?sid=" + this.sid, function(data){
								if (data) {
									alert(data)
									return;							
								}
...
function openTmplEditDialog(isrelease) {
	...
	var dialogbtn = {
			...
			'确定':function(){
				if ($("[name=tmplneweditform]").validate()) {
					var htmltable = encodeURIComponent(encodeURIComponent(tidyhtmltable($(".report-table").clone(true))));
					$.post(ctxPath + "/report/addnewtmpl.vot?", encodeURI($("[name=tmplneweditform]").serialize())+"&htmltable="+htmltable+"&groupsid="+(options.groupsid||''), function(){
	...
	$("#uploadexcel").loadFileUpDownloader ({wmode:"transparent", htmlText:"<font color='#005590' face='宋体'><u>上传excel</u></font>", single:true, extension:"*.xls"}, function(){
		this.selectFile (function (filelist) {
			...
			var managernamestr = "";
			if (options.excelmanager) {
				managernamestr = "&managername=" + options.excelmanager;
			}
			return ctxPath + "/report/uploadexcel.vot?guid=" + guid + managernamestr;
		}).uploadComplete (function (info) {
			$.get(ctxPath + "/report/gethtmltable.vot?guid=" + guid, function(data) {
				if (data != 'null') {
					setHtmltable(data);
				} else {
					alert("您上传的模板格式不正确,请编辑后重新上传!");
				}
			});
		});
	});
...
function setHtmltable(data) {
	data = data||'';
	$("#htmltable_container").html(data).find(".report-table").selectable({
		// 只允许选择空单元格或者是含有div(含有状态提示信息)的单元格
		filter:"td:empty, td[fixed!=true]:has(div)", stop: function() {
			var edittype;
			var sumtype;
			...
			if (!sumtypenotsame) {
				if (sumtype)
					$("[name=sumtype]").filter("[value="+sumtype.value+"]").prop("checked", true).button("refresh");
				else
	...
	// 恢复data属性
		...
		var self = $(this);
		var attrs = ['edittype','dict', 'sumtype'];
		for (var j = 0; j < attrs.length; j++) {
			var att = self.attr(attrs[j]);
			if (att != null) {
				if (att.charAt(0) == '%') { // 为对象，因为对象被序列化了
					att = $.parseJSON(decodeURI(att));
				}
				self.data(attrs[j], att);
				self.removeAttr(attrs[j]);
			}
		}
	});
}
...
function addDictRelate(thisObj, buttonindex) {
	...
	// 选中的表格单元格
	var selectedcell = $(".report-table .ui-selected");
...
function resetTableCellInfo() {
	$(".report-table .ui-selected").each(function(){
		var self = $(this);
		self.empty();
...
function exportReportTemplate() {
	$.postIframe(ctxPath + "/report/exportExcelTmpl.vot?managername=" + options.excelmanager+"&s="+Math.random());
}
	...
	<div id="creatediv" style="display: none;">
		...
		<div id="statebuttonbar">
			...
			<span style="display: inline-block; padding-right: 2px;">
				<button id="exporttmpl" class="smallbutton" onclick="exportReportTemplate()">导出标准模板</button>
