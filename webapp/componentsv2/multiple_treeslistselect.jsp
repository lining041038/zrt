...
<%
MultipleTreesListSelectionOptions opts = (MultipleTreesListSelectionOptions) request.getAttribute("opts");
...
function dialogTopIframeShowed(){
	...
	for (var i = 0; i < <%=entries.size()%>; ++i) {
		westsizes.push(190 / bodywidth);
	}
	
	// 左侧列表
	westsizes.push ((bodywidth - 190 * <%=entries.size()%> - 90) / 2 / bodywidth);
	
	// 操作按钮列
	westsizes.push (90 / bodywidth);
	...
	$.getJSON(ctx + "/common/componentsv2/MultipleTreesListSelectController/getTreesListSelectOptions.vot?component=" + comp, function(data){
		...
		$("#leftdatagrid").datagrid({
			dataProvider:ctx + "/common/componentsv2/MultipleTreesListSelectController/getListSelectItemsToSelect.vot?component=" + comp,
			columns: ncolumns,
			pagebuttonnum: 0,
		...
		if (selectedItemSids || parentsid) {
			$.post(ctx + "/common/componentsv2/MultipleTreesListSelectController/getHistSelectedItems.vot?component=" + comp, "selectedItemSids=" + selectedItemSids + "&parentsid=" + parentsid, function (data){
...
%>
	function createLeftGridTree<%=i%>(prevtreesid) {
		...
		var oneTree = $("#treeul<%=i%>").tree("destroy").tree({
			asyncUrl: ctx + "/common/componentsv2/MultipleTreesListSelectController/getTreesListSelectItemsToSelect.vot?component=" + comp + "&treekey=<%=entry.getKey()%>" + 
			...
			click: function() {
				...
				if (treeidx == (<%=entries.size()%> - 1)) {
					$("#leftdatagrid").datagrid("option", "dataProvider", ctx + "/common/componentsv2/MultipleTreesListSelectController/getListSelectItemsToSelect.vot?component=" + comp + "&treenode_sid=" + treeobj.tree("getSelectedNode").sid);
				} else {
...
<%if (i >= entries.size() - 1) {%>
	<div class="col1<%=(i+1)%>">
		<div class="ui-widget ui-widget-content" style="height: 100%; margin-top: 2px;">
			...
			<div id="leftdatagrid" class="heightsolidcol"></div>
		</div>
	</div>
	<div class="col2<%=(i+1)%>">
		<div class="col1<%=(i+2)%>">
