...
import antelope.wcm.entities.WCMArticleCateItem;
import antelope.wcm.entities.WCMArticleItem;
...
@RequestMapping("/wcm/articlemanage/ArticleManageController")
public class ArticleManageController extends TreeDatagrid {
	...
	public TreeDatagridOptions getOptions(HttpServletRequest req) {
		...
		opts.columns.put("authorname", new GridColumn("作者", "30%"));
	...
	public void getSingleGridList(String name, HttpServletRequest req, HttpServletResponse res) throws SQLException, Exception {
		...
		PageItem json = articleservice.getArticlesPageDataByCatesid(treenode_sid, getPageParams(req), sqlwhere);
		print(json, res);
	}
	
	
	@RequestMapping("/getArticleContent")
	public void getArticleContent(String sid, HttpServletRequest req, HttpServletResponse res) throws SQLException, Exception {
		print(DBUtil.querySingleString("select content from WCM_ARTICLE where sid=?", new Object[]{sid}), res);
	}
	...
	public void getChildren(String sid, HttpServletRequest req, HttpServletResponse res) throws SQLException, Exception {
		if (!stringSet(sid)) {
			...
			JSONArray arr = toJSONArray(DBUtil.queryJSON("select * from WCM_ARTICLE_CATE where parentsid=?", sid));
	...
	public void batchDeleteLines(String sids, HttpServletRequest req, HttpServletResponse res) throws InstantiationException, IllegalAccessException {
		dao.updateBySQL("delete from WCM_ARTICLE where sid in (" + createQuestionMarksStr(sids.split(",").length) + ")", sids.split(","));
	}
	...
	public void addOrUpdateOneTreeNode(String sid, HttpServletRequest req, HttpServletResponse res) throws Exception {
		WCMArticleCateItem item = dao.getBy(sid, WCMArticleCateItem.class);
		if (item == null)
			item = newInstanceWithCreateInfo(WCMArticleCateItem.class, req);
	...
	public void deleteOneTreeNode(String sid, HttpServletRequest req, HttpServletResponse res) throws Exception {
		...
		if (DBUtil.queryCount("select count(*) from WCM_ARTICLE_CATE where parentsid=?", sid) > 0) {
			print("此分类还有子分类，不允许删除！", res);
			return;
		}
		
		if (DBUtil.queryCount("select count(*) from WCM_ARTICLE where catesid=?", sid) > 0) {
			print("此分类还有文章，不允许删除！", res);
			return;
		}
		
		dao.deleteBy(sid, WCMArticleCateItem.class);
	}
	...
	public void deleteOneLine(String sid, HttpServletRequest req, HttpServletResponse res) throws InstantiationException, IllegalAccessException {
		...
		if (sids != null)
			dao.updateBySQL("delete from WCM_ARTICLE where sid in(" + createQuestionMarksStr(sids.length) + ")", sids);
	}
	...
	public void addOrUpdateOne(String sid, HttpServletRequest req, HttpServletResponse res) throws Exception {
		WCMArticleItem item = dao.getBy(sid, WCMArticleItem.class);
		if (item == null)
			item = newInstanceWithCreateInfo(WCMArticleItem.class, req);
