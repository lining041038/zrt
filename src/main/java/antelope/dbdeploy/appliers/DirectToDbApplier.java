...
import antelope.dbdeploy.ChangeScriptApplier;
...
import antelope.dbdeploy.exceptions.ChangeScriptFailedException;
...
public class DirectToDbApplier implements ChangeScriptApplier {
    ...
    public void apply(List<ChangeScript> changeScript) {
        ...
        for (ChangeScript script : changeScript) {
            System.out.println("Applying " + script + "...");
		...
		for (int i = 0; i < statements.size(); i++) {
			...
			try {
				if (statements.size() > 1) {
					System.out.println(" -> statement " + (i+1) + " of " + statements.size() + "...");
				}
