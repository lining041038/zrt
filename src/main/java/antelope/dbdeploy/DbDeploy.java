...
import antelope.dbdeploy.appliers.DirectToDbApplier;
import antelope.dbdeploy.appliers.TemplateBasedApplier;
import antelope.dbdeploy.appliers.UndoTemplateBasedApplier;
import antelope.dbdeploy.database.DelimiterType;
import antelope.dbdeploy.database.LineEnding;
import antelope.dbdeploy.database.QueryStatementSplitter;
import antelope.dbdeploy.database.changelog.DatabaseSchemaVersionManager;
import antelope.dbdeploy.database.changelog.QueryExecuter;
import antelope.dbdeploy.exceptions.UsageException;
import antelope.dbdeploy.scripts.ChangeScript;
import antelope.dbdeploy.scripts.ChangeScriptRepository;
import antelope.dbdeploy.scripts.DirectoryScanner;
...
public class DbDeploy {
	...
	public void go() throws Exception {
		...
		QueryExecuter queryExecuter = new QueryExecuter(ds);
		...
		DirectoryScanner dictscan = new DirectoryScanner(encoding);
		List<ChangeScript> changedscripts = dictscan.getChangeScriptsForDirectory(scriptdirectory);
		ChangeScriptRepository changeScriptRepository = new ChangeScriptRepository(changedscripts);
