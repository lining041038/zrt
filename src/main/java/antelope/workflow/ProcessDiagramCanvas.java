...
public class ProcessDiagramCanvas {
  ...
  public void drawStartEvent(int x, int y, int width, int height, Image image) {
    ...
    List<String> retStrings = fitTextToWidth("开始", width);
     int textX = x + ((width - fontMetrics.stringWidth(retStrings.isEmpty()? "" : retStrings.get(0))) / 2);
     int textY = y + ((height - fontMetrics.getHeight() * retStrings.size()) / 2) + fontMetrics.getHeight() - 2;
     for (int i = 0; i < retStrings.size(); ++i) {
   	  g.drawString(retStrings.get(i), textX, textY + i * 15);
     }
  ...
  public void drawNoneEndEvent(int x, int y, int width, int height) {
    ...
    List<String> retStrings = fitTextToWidth("结束", width);
    int textX = x + ((width - fontMetrics.stringWidth(retStrings.isEmpty()? "" : retStrings.get(0))) / 2);
    int textY = y + ((height - fontMetrics.getHeight() * retStrings.size()) / 2) + fontMetrics.getHeight() - 2;
    for (int i = 0; i < retStrings.size(); ++i) {
  	  g.drawString(retStrings.get(i), textX, textY + i * 15);
  ...
  protected void drawTask(String name, int x, int y, int width, int height, boolean thickBorder) {
    ...
    // text
    if (name != null) {
     List<String> retStrings = fitTextToWidth(name, width);
      int textX = x + ((width - fontMetrics.stringWidth(retStrings.isEmpty()? "" : retStrings.get(0))) / 2);
      int textY = y + ((height - fontMetrics.getHeight() * retStrings.size()) / 2) + fontMetrics.getHeight();
      for (int i = 0; i < retStrings.size(); ++i) {
    	  g.drawString(retStrings.get(i), textX, textY + i * 15);
      }
    }
  ...
  protected List<String> fitTextToWidth(String original, int width) {
    String text = original;

    // remove length for "..."
    int maxWidth = width - 10;
    List<String> retStrings = new ArrayList<String>();
    StringBuffer lefttexts = new StringBuffer();
    
    while (text.length() > 0) {
	    while (fontMetrics.stringWidth(text + "...") > maxWidth && text.length() > 0) {
	    	lefttexts.insert(0, text.charAt(text.length() - 1));
	    	text = text.substring(0, text.length() - 1);
	    }
	    retStrings.add(text);
	    text = lefttexts.toString();
	    lefttexts = new StringBuffer();
    }
  ...
  public void drawExpandedSubProcess(String name, int x, int y, int width, int height) {
    ...
    List<String> text = fitTextToWidth(name, width);
    for (int i = 0; i < text.size(); ++i) {
    	g.drawString(text.get(i), x + 10, y + 15 + 15 * i);
    }
  }
